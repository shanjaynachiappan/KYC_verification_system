...
import { uploadSelfieForQualityCheck, matchFace } from '@/services/apiClient';
...
export default function SelfieScreen() {
  ...
  const handleCapture = async () => {
    ...
    if (status !== 'granted') {
      setFailureReason('Camera permission is required for real selfie verification.');
      return;
    }
    ...
    const result = await ImagePicker.launchCameraAsync({
      ...
      base64: true, // needed for the /face/match JSON body
    });
  ...
  const handleVerify = async () => {
    ...
    if (!user?.userId || !photoUri || !photoBase64) {
      setFailureReason('Missing session or photo data. Please retake the selfie.');
      setStage('failed');
    ...
    try {
      // Step 1: OpenCV quality gate (blur / overexposure / face-count) -- Member A
      const quality = await uploadSelfieForQualityCheck(photoUri);
      if (quality.status !== 'accepted') {
        setMatchScore(0);
        setFailureReason(quality.message || 'Selfie failed the quality check. Please retake in better lighting.');
        setStage('failed');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
        return;
      }

      // Step 2: real DeepFace ArcFace match vs the Aadhaar photo fetched via DigiLocker -- Member B
      const result = await matchFace(user.userId, photoBase64);
      const score = result.similarity_score ?? (result.matched ? 1 : 0);
      setMatchScore(score);
      ...
      if (result.matched) {
        ...
        setFailureReason(result.quality_issue || 'Face did not match your DigiLocker Aadhaar photo.');
        setStage('failed');
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
      }
    } catch (e) {
      setFailureReason(e instanceof Error ? e.message : 'Face match request failed.');
      setStage('failed');
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
    }
  };
        ...
        <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: bottomPad + 32 }]} showsVerticalScrollIndicator={false}>
          ...
          <Text style={[styles.resultSubtitle, { color: colors.mutedForeground }]}>
            {passed
              ...
              : failureReason || 'We could not match your selfie. Please retake in good lighting.'}
          </Text>
