...
import { uploadDocumentForOcr, OcrUploadResponse } from '@/services/apiClient';
...
export default function DocumentsScreen() {
  ...
  const handlePickDoc = async () => {
    ...
    if (status !== 'granted') {
      ...
      setOcrResult({
        status: 'rejected',
        message: 'Media library permission denied -- cannot run real OCR without an actual image.',
      });
      return;
    }

    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ImagePicker.MediaTypeOptions.Images, // OCR endpoint only accepts JPG/PNG, not arbitrary PDFs
      allowsEditing: false,
      quality: 0.9,
    });
    ...
    setLoading(true);
    try {
      const ocr = await uploadDocumentForOcr(asset.uri, name, mimeType);
      setOcrResult(ocr);
      ...
      setOcrResult({
        status: 'rejected',
        message: e instanceof Error ? e.message : 'OCR request failed -- check the backend is running and reachable.',
      });
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Error);
    } finally {
      setLoading(false);
  ...
  const handleContinue = async () => {
    if (!uploaded || ocrResult?.status !== 'success') return;
    setLoading(true);
    ...
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      >
        ...
        {!uploaded ? (
          ...
          <View style={[styles.uploadedCard, { backgroundColor: colors.card, borderColor: ocrResult?.status === 'success' ? colors.success : colors.destructive }]}>
            <View style={[styles.uploadedLeft, { backgroundColor: ocrResult?.status === 'success' ? colors.successLight : colors.card, borderColor: ocrResult?.status === 'success' ? colors.success : colors.destructive }]}>
              <Ionicons name="document-text" size={28} color={ocrResult?.status === 'success' ? colors.success : colors.destructive} />
            ...
            <View style={styles.uploadedInfo}>
              ...
              <Text style={[styles.uploadedType, { color: ocrResult?.status === 'success' ? colors.success : colors.destructive }]}>
                {ocrResult?.status === 'success' ? 'OCR extraction succeeded' : (ocrResult?.message || (loading ? 'Running OCR...' : 'Waiting to upload'))}
              </Text>
            </View>
        ...
        {/* Real OCR result -- honest about the current backend gap: identify_document()
            only recognizes AADHAAR/PAN today. Payslip/bank-statement/ITR/utility/rent
            docs will come back "rejected: unsupported document" until that's extended
            (see document_parser.py). We surface whatever the backend actually returned
            rather than fabricating extracted fields. */}
        {uploaded && ocrResult && (
          <View style={[styles.ocrPreviewCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
            <View style={styles.ocrPreviewHeader}>
              ...
              <Text style={[styles.ocrPreviewTitle, { color: colors.primary }]}>OCR RESULT (LIVE)</Text>
            </View>
            <OcrRow label="Status" value={ocrResult.status} colors={colors} />
            <OcrRow label="Message" value={ocrResult.message} colors={colors} />
            {typeof ocrResult.is_blurry === 'boolean' && (
              <OcrRow label="Blur check" value={ocrResult.is_blurry ? 'Blurry' : 'Sharp'} colors={colors} />
            )}
            {typeof ocrResult.is_overexposed === 'boolean' && (
              <OcrRow label="Exposure check" value={ocrResult.is_overexposed ? 'Overexposed' : 'OK'} colors={colors} />
            )}
            {ocrResult.full_text && (
              <OcrRow label="Extracted text" value={ocrResult.full_text.slice(0, 120)} colors={colors} />
            )}
            {ocrResult.status !== 'success' && (
              <View style={[styles.ocrFooter, { borderTopColor: colors.border }]}>
                <Ionicons name="information-circle-outline" size={13} color={colors.destructive} />
                <Text style={[styles.ocrFooterText, { color: colors.destructive }]}>
                  This document type isn't recognized by the current OCR service yet -- only Aadhaar/PAN are identified today.
                </Text>
              </View>
            )}
          </View>
        ...
        <PrimaryButton
          ...
          onPress={!uploaded || ocrResult?.status !== 'success' ? handlePickDoc : handleContinue}
          loading={loading}
          disabled={false}
        />
      </ScrollView>
